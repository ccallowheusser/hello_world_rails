class BlogPostsController < ApplicationController
  # GET request
  # listing all blog posts
  def index
    @blog_posts = BlogPost.all  # needs instance variable so view has access
    # render :index   # not necessary but this is what it does
  end

  # fetching with GET request
  # showing a single blog post
  def show
    @blog_post = BlogPost.find(params[:id])
  end

  # GET request
  # showing the user a form to fill out for a new blog post
  def new
    @blog_post = BlogPost.new
  end

  # POST request (to create)
  # write new, respondible for creating a ndw blog post record
  def create
    @blog_post = BlogPost.new(blog_post_params)
    if @blog_post.save
      # got the new record in the database
      redirect_to blog_post_path(@blog_post)
    else
      # something went wrong, have suer fix errors on the form
      render :new
    end
  end

  # GET request
  # showing the user a form to edit an existing blog post
  def edit
    @blog_post = BlogPost.find(params[:id])
  end

  # PUT or PATCH request
  # new code, finding the record to update and update it
  def update
    @blog_post = BlogPost.find(params[:id])
    if @blog_post.update(blog_post_params)
      redirect_to blog_post_path(@blog_post)
    else
      render :edit
    end
  end

  # DELETE request
  # in rails, delete is called destroy
  def destroy
    @blog_post = BlogPost.find(params[:id])
    @blog_post.destroy
    redirect_to blog_posts_path
  end

  # Strong Params
  private
    def blog_post_params
      params.require(:blog_post).permit(:title, :author, :body)
    end
end
